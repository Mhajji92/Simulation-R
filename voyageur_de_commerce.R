...
## Le problème du voyageur de commerce, est un problème  d'optimisation qui, étant
## donné une liste de villes, et des distances entre toutes les paires de  villes, détermine
## un plus court chemin qui visite chaque ville une et une seule fois et qui termine dans la ville
## de départ.Le problème du voyageur de commerce  étant NP-Complet, n'est pas resolvable dans un temps
## Polynomial, Nous proposons dans cette exercice une optimisation par l'algorithme de Metrpolis.
...
transition1 = function(G,beta) {
  result=G
  Gnew=G
  k0=sample(1:N,2,replace=F)
  Gnew[k0[1]]=G[k0[2]]
  Gnew[k0[2]]=G[k0[1]]
  #Test (accepter de remonter)
  delta=0
  for (i in 1:(N-1)){
  delta= delta + abs((V[Gnew[i]+1,Gnew[i+1]+1]))-abs((V[G[i]+1,G[i+1]+1]))
  }
...
transition2 = function(G,beta) {
  ...
  Gnew=G
  k0=sample(1:N,2,replace=F)
  for (i in 0:(abs(k0[2]-k0[1]))) {
  Gnew[min(k0[1],k0[2])+i]=G[max(k0[1],k0[2])-i]
  }
  #Test (accepter de remonter)
  delta=0
  for (i in 1:(N-1)){
